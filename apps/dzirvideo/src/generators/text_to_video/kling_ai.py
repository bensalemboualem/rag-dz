"""
Kling AI 2.0 - Text-to-Video Generator
Kuaishou's high-quality video generation with 66 free credits/day
Official docs: https://docs.klingai.com/
"""
...
class KlingAIGenerator(BaseGenerator):
    """
    Kling AI 2.0 Text-to-Video Generator (via Replicate)

    Features:
    - 66 free credits per day
    - Max 120 seconds per video
    - High quality (Quality Score: 90/100)
    - Multiple aspect ratios
    - Professional results

    Note: Uses Replicate API as wrapper for Kling AI
    """

    def __init__(self, api_key: Optional[str] = None, **kwargs):
        """
        Initialize Kling AI generator
        ...
        self.api_key = api_key or os.getenv("REPLICATE_API_TOKEN") or os.getenv("KLING_AI_API_KEY")
        if not self.api_key:
            raise ValueError(
                "Replicate API token required for Kling AI. "
        ...
        # Kling AI model on Replicate
        self.model_version = "kuaishou/kling-ai:latest"

        logger.info("Kling AI generator initialized")

    def _define_capabilities(self) -> GeneratorCapabilities:
        """Define Kling AI capabilities"""
        return GeneratorCapabilities(
            # Support
            supports_text_to_video=True,
            supports_image_to_video=True,  # Kling supports image-to-video too
            ...
            # Specs
            max_duration_seconds=120.0,  # 2 minutes max
            ...
            # Pricing (FREEMIUM)
            api_cost_per_second=0.01,  # $0.01/sec after free credits
            api_cost_per_image=0.0,
            free_tier=True,
            free_credits_per_day=66,  # ~10 videos per day
            ...
            # Performance
            avg_generation_time_seconds=60.0,  # ~1 minute
            supports_async=True,
    ...
    async def generate(self, request: GenerationRequest) -> GenerationResult:
        ...
        try:
            ...
            logger.info(f"Kling AI generation started: task_id={task_id}")
            ...
            if "quota" in error_msg.lower() or "rate limit" in error_msg.lower():
                raise QuotaExceededError(f"Kling AI quota exceeded: {error_msg}")

            raise APIError(f"Kling AI API error: {error_msg}")

        except Exception as e:
            logger.error(f"Error calling Kling AI: {e}")
            raise APIError(f"Kling AI generation failed: {str(e)}")
    ...
    async def check_status(self, task_id: str) -> GenerationResult:
        ...
        try:
            ...
            if status == GenerationStatus.COMPLETED:
                ...
                # Kling AI returns video URL directly
                ...
                if result.output_url:
                    logger.info(f"Kling AI generation completed: {result.output_url}")
    ...
    async def cancel(self, task_id: str) -> bool:
        ...
        try:
            ...
            if prediction.status in ["starting", "processing"]:
                ...
                logger.info(f"Kling AI task {task_id} cancelled")
                return True

            return False

        except Exception as e:
            logger.error(f"Error cancelling Kling AI task: {e}")
            return False
    ...
    def _build_api_params(self, request: GenerationRequest) -> Dict:
        ...
        if request.duration_seconds:
            duration = min(request.duration_seconds, 120.0)
    ...
    def __str__(self) -> str:
        return "KlingAIGenerator(Kuaishou, Freemium, Quality=90)"
