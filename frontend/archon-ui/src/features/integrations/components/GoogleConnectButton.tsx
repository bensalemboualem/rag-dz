/**
 * GoogleConnectButton Component
 * OAuth2 connect/disconnect button for Google
 */

import { useState, useEffect } from 'react';
import { Loader2, LogOut, Check } from 'lucide-react';
import {
  initGoogleAuth,
  disconnectGoogle,
  getStoredAuthState,
  isGoogleConnected,
} from '../services/googleService';
import type { GoogleAuthState } from '../types';

// Google Logo SVG
const GoogleLogo = () => (
  <svg viewBox="0 0 24 24" className="w-5 h-5">
    <path
      fill="#4285F4"
      d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
    />
    <path
      fill="#34A853"
      d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
    />
    <path
      fill="#FBBC05"
      d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
    />
    <path
      fill="#EA4335"
      d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
    />
  </svg>
);

interface GoogleConnectButtonProps {
  onConnect?: (state: GoogleAuthState) => void;
  onDisconnect?: () => void;
  className?: string;
}

export function GoogleConnectButton({
  onConnect,
  onDisconnect,
  className = '',
}: GoogleConnectButtonProps) {
  const [loading, setLoading] = useState(false);
  const [connected, setConnected] = useState(false);
  const [authState, setAuthState] = useState<GoogleAuthState | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const state = getStoredAuthState();
    if (state?.isConnected) {
      setConnected(true);
      setAuthState(state);
    }
  }, []);

  const handleConnect = async () => {
    setLoading(true);
    setError(null);

    try {
      const state = await initGoogleAuth();
      setConnected(true);
      setAuthState(state);
      onConnect?.(state);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to connect');
    } finally {
      setLoading(false);
    }
  };

  const handleDisconnect = async () => {
    setLoading(true);

    try {
      await disconnectGoogle();
      setConnected(false);
      setAuthState(null);
      onDisconnect?.();
    } catch (err) {
      console.error('Failed to disconnect:', err);
    } finally {
      setLoading(false);
    }
  };

  if (connected && authState) {
    return (
      <div className={`flex items-center gap-3 ${className}`}>
        <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-500/10 border border-emerald-500/30">
          <Check className="w-4 h-4 text-emerald-400" />
          <span className="text-sm text-emerald-400">Connecté</span>
          {authState.email && (
            <span className="text-xs text-gray-500">({authState.email})</span>
          )}
        </div>
        <button
          onClick={handleDisconnect}
          disabled={loading}
          className="p-2 rounded-lg hover:bg-red-500/10 text-gray-500 hover:text-red-400 transition-colors"
          title="Déconnecter"
        >
          {loading ? (
            <Loader2 className="w-4 h-4 animate-spin" />
          ) : (
            <LogOut className="w-4 h-4" />
          )}
        </button>
      </div>
    );
  }

  return (
    <div className={className}>
      <button
        onClick={handleConnect}
        disabled={loading}
        className="inline-flex items-center gap-3 px-4 py-2 rounded-lg bg-white dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-700 transition-colors shadow-sm"
      >
        {loading ? (
          <Loader2 className="w-5 h-5 animate-spin text-gray-500" />
        ) : (
          <GoogleLogo />
        )}
        <span className="text-sm font-medium text-gray-700 dark:text-gray-200">
          {loading ? 'Connexion...' : 'Se connecter avec Google'}
        </span>
      </button>
      {error && (
        <p className="mt-2 text-xs text-red-400">{error}</p>
      )}
    </div>
  );
}
