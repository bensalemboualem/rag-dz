...
export function MainLayout({ children, className }: MainLayoutProps) {
  ...
  return (
    <div className={cn("relative min-h-screen bg-black overflow-x-hidden", className)}>
      {/* TEMPORARY: Show backend startup error using old component */}
      {backendStartupFailed && <BackendStartupError />}

      {/* Bolt.new Style Header */}
      <BoltHeader />

      {/* Fixed full-page background with blue glow effect */}
      <div className="fixed inset-0 pointer-events-none z-0">
        {/* Grid background */}
        <div className="absolute inset-0 neon-grid opacity-30" />

        {/* Blue glow effect at bottom (signature Bolt.new) */}
        <div
          className="absolute bottom-0 left-0 right-0 h-[400px]"
          style={{
            background: 'radial-gradient(ellipse 100% 50% at 50% 100%, rgba(59, 130, 246, 0.25) 0%, rgba(59, 130, 246, 0.1) 40%, transparent 70%)'
          }}
        />
      </div>

      {/* Floating Navigation (kept for quick access) */}
      ...
      {/* Main Content Area - avec padding top pour le header fixed */}
      <div className="relative flex-1 pl-[100px] z-10">
        <div className="container mx-auto px-8 relative">
          <div className="min-h-screen pt-24 pb-16">{children}</div>
        </div>
      </div>

      {/* Bolt.new Style Footer */}
      <BoltFooter />
      ...
      <div className="fixed bottom-6 right-6 z-50 group">
        <button
          type="button"
          disabled
          className="w-14 h-14 rounded-full flex items-center justify-center backdrop-blur-md bg-gradient-to-b from-gray-700/30 to-gray-800/30 shadow-[0_0_10px_rgba(59,130,246,0.3)] cursor-not-allowed opacity-60 overflow-hidden border border-white/10"
          aria-label="Knowledge Assistant - Coming Soon"
        >
          <img src="/logo-neon.png" alt="Archon" className="w-7 h-7 grayscale opacity-50" />
        </button>
        {/* Tooltip */}
        <div className="absolute bottom-full right-0 mb-2 px-3 py-2 bg-gray-800/98 backdrop-blur-sm text-white text-sm rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap border border-white/10">
          <div className="font-medium">Coming Soon</div>
          <div className="text-xs text-white/70">Knowledge Assistant is under development</div>
          <div className="absolute bottom-0 right-6 transform translate-y-1/2 rotate-45 w-2 h-2 bg-gray-800 border-l border-b border-white/10"></div>
        </div>
      </div>
    </div>
  );
